...
original_order_date_full_str = target_sale.get("OrderDate")
if not original_order_date_full_str:
    print(f"[SKIP] Sale found but has no OrderDate in SaleList data.")
    exit()
print(f"[INFO] Original OrderDate from SalesList (full string): {original_order_date_full_str}")

# Extract just the date part for the SaleOrderDate field
original_order_date_simple = original_order_date_full_str.split("T")[0]
print(f"[INFO] Prepared simple date for SaleOrderDate: {original_order_date_simple}")
...
# === STEP 4: Inject OrderDate into the correct AdditionalAttributeX and Customer Info ===
# We are now assuming the UI field is a custom attribute (e.g., AdditionalAttribute1)
...
# REPLACE 'AdditionalAttributeX' with the actual field name you found in Step 1
# For example, if it was 'AdditionalAttribute1':
sale_data["AdditionalAttributes"]["AdditionalAttribute2"] = original_order_date_simple # Use the simple date string
print(f"[INFO] Set 'AdditionalAttributes.AdditionalAttribute1' to: {original_order_date_simple}")

# You can remove the SaleOrderDate line if you are certain the UI isn't using it
# del sale_data["SaleOrderDate"] # Or set it to null if that's safer for your API
# print("[INFO] Removed SaleOrderDate as it's not the UI-visible field.")


# Inject CustomerID or Customer Name (this part remains the same)
...
debug_payload = {
    ...
    "SaleOrderNumber": sale_data.get("SaleOrderNumber"),
    "SaleOrderDate": sale_data.get("SaleOrderDate"), # Check this value
    ...
    "Lines_Count": len(sale_data.get("Lines", []))
}
...
try:
    ...
    if put_response.status_code == 200:
        print(f"[SUCCESS] Sale {SALE_ID_TO_FIND} updated successfully with SaleOrderDate = {original_order_date_simple}")
    else:
